...
const controlOrdersButtons = async (ctx, next) => {
  ...
  const currentPage = Number.parseInt(keyboard[keyboard.length - 1][1].callback_data.split(' ')[0], 10);
  const totalPages = Number.parseInt(keyboard[keyboard.length - 1][1].callback_data.split(' ')[2], 10);
...
const getOrderCustomer = async (ctx, next) => {
  ...
  const { customer } = ctx.session.currentOrder;
  ...
  await ctx.editMessageText(
      ...
      `🔑 <b>Admin: </b><i>${customer.isAdmin}</i>\n`,
    {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [[Markup.callbackButton('⬅ Back', actions.BACK_TO_ORDER)]]
      }
    }
  );
};
...
const getOrderProducts = async (ctx, next) => {
  ...
  const limitedProducts = allProducts.slice(0, process.env.MAX_ITEMS_PER_PAGE);

  const pagesAmount = Math.ceil(allProducts.length / process.env.MAX_ITEMS_PER_PAGE);
  await ctx.reply('📦 <b>Products list: </b> \n\n', {
    reply_markup: {
      inline_keyboard: [
...
const controlOrderProductsButtons = async (ctx, next) => {
  ...
  const currentPage = Number.parseInt(keyboard[keyboard.length - 2][1].callback_data.split(' ')[0], 10);
  const totalPages = Number.parseInt(keyboard[keyboard.length - 2][1].callback_data.split(' ')[2], 10);
  ...
  const newProducts = ctx.session.currentOrder.products.slice(
    (nextPage - 1) * process.env.MAX_ITEMS_PER_PAGE,
...
const getProductById = async (ctx, next) => {
  ...
  await ctx.reply(
    ...
    {
      reply_markup: {
        inline_keyboard: [[Markup.callbackButton('⬅ Back', actions.BACK_TO_PRODUCTS)]]
      },
      parse_mode: 'HTML'
    }
  );
