...
const sendCurrentBeatState = async (beat, ctx, cursor) => {
    const caption = `📋 <b>${beat?.title || 'Empty'}</b>\n\n` +
        `🎹 <b>BPM:</b> ${beat?.bpm || 'Empty'}\n` +
        `🎵 <b>Scale:</b> ${beat?.scale || 'Empty'}\n` +
        `🏷️ <b>Tags:</b> ${beat?.tags?.replace(/\\s/g, '').split(',').map(t => '#' + t).join(' ') || 'Empty'}\n` +
        `🪕 <b>Genres:</b> ${beat?.genres?.replace(/\\s/g, '').split(',').map(t => '#' + t).join(' ') || 'Empty'}\n` +
    ...
    let messageId;
    if (beat?.imgUrl) {
        const {message_id} = await ctx.replyWithPhoto({source: fs.createReadStream(beat.imgUrl)}, {
            caption: caption,
            parse_mode: 'HTML',
            reply_markup: {
                inline_keyboard: [
                    [Markup.callbackButton('🚫 Cancel', 'cancel')],
                    [Markup.callbackButton('🌀 Listen', 'listen')],
                ]
            }
        })
...
const createNewBeatScene = new WizardScene(sceneNames.CREATE_NEW_BEAT_SCENE,
    async ctx => {
