...
export default class BeatstoreService {
  ...
  getInfo = async () => axios.get(`${this.baseUrl}/info`);

  getBeatById = async (id) => axios.get(`${this.baseUrl}/${id}`);

  getRandomBeat = async () => axios.get(`${this.baseUrl}/random`);

  downloadBeat = async (id, handleProgress) =>
    axios.get(`${this.baseUrl}/download` + `/${id}`, {
      responseType: 'blob',
  ...
  removeLike = async (commentId, userId, token) =>
    axios.delete(`${this.baseUrl}/like/${commentId}/${userId}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
