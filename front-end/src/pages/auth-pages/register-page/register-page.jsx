...
import { googleContinue, googleContinueFailed, signup, userErrorCleared } from '../../../redux/actions';
...
function RegisterPage({ authService }) {
  ...
  const confirmedMsg =
    !formState.confirmed && touched ? (
      <p className="confirm-pass-error-msg">Password and confirmed password should be identical</p>
    ) : null;

  const alreadyExistsError =
    userState.error && !formState.isValid ? <p className="confirm-pass-error-msg">{userState.error.message}</p> : null;
  ...
  return (
    <div className="auth-page">
      ...
      {hasRegistered && !userState.error ? (
        <SecondRegistrationStep />
      ) : (
        <>
          <h2>Register</h2>
          <form onBlur={() => setTouched(true)} className="email-login-form" onSubmit={onUserSignupHandler}>
          ...
          <GoogleLogin
            ...
            render={(renderProps) => (
              <button className="oauth" onClick={renderProps.onClick} disabled={renderProps.disabled}>
                <img
          ...
          />
          <p className="transfer-to-login">
            Already have an account?
            <Link to="/auth/login" onClick={(e) => dispatch(userErrorCleared())}>
              &nbsp;Sign in
            </Link>
          </p>
        </>
