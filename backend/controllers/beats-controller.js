...
const getBeatById = async (req, res, next) => {
  ...
  if (!beat) {
    return next(new HttpError("Couldn't find a beat by specified id!", 404));
  }
  ...
  res.json({
    message: 'Successfully found a beat',
    beat: {
...
const updateBeatById = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError('Something went wrong while trying to find a beat..'), 500);
  }

  if (!updatedBeat) {
    return next(new HttpError("Such beat hasn't been found!"), 403);
  }
  ...
  try {
    ...
    return next(new HttpError('Something went wrong while saving updated beat to database..'), 500);
  }
...
const deleteBeat = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError("Couldn't find a beat with this id.."), 500);
  }

  let users;
  try {
    users = await User.find({ 'cart.items': { $elemMatch: { beatId } } });
    await Promise.all(
      users.map(async (u) => {
  ...
  try {
    ...
    return next(new HttpError("Couldn't delete a beat from database.."), 500);
  }
...
const getUniqueInfo = async (req, res, next) => {
  ...
  try {
    ...
    console.log(e.message);
    return next(new HttpError('Something went wrong while getting info about beats', 500));
  }
  const bpms = [];
  const moods = [];
  const genres = [];
...
const getRandomBeat = async (req, res, next) => {
  ...
  try {
    ...
    console.log(e.message);
    return next(new HttpError('Something went wrong while getting random beat', 500));
  }

  res.status(200);
  res.json({
    message: 'Successfully retrieved random beat',
    beat: beats.map((b) => ({
      ...b,
      id: b._id.toString()
    }))[0]
...
const leaveComment = async (req, res, next) => {
  ...
  if (!beat || !user) {
    return next(new HttpError("Couldn't find a beat or user by specified ids!", 404));
  }
  ...
  res.status(200);
  res.json({
    message: 'Successfully left a comment',
    comment: {
