...
const getUserById = async (req, res, next) => {
  ...
  if (!user) {
    return next(new HttpError("Couldn't find a user with specified id!", 404));
  }
...
const getAllUsers = async (req, res, next) => {
  ...
  try {
    ...
    users = await User.find({}, '-password', { skip }).limit(limit);
  } catch (e) {
...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError('Signing up has failed..', 500));
  }

  if (existingUser) {
    return next(new HttpError('User with such email already exists', 409));
  }
  ...
  let userToSend;
  const messageToSend = 'Check your email to verify your account';
  ...
  const confirmationToken = jwt.sign({ email, username }, process.env.CONFIRMATION_TOKEN_SECRET + newUser._id.toString());
  ...
  try {
    ...
    return next(new HttpError('Something went wrong while saving new user to a database..', 500));
  }
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError('Logging in has failed..', 500));
  }
  ...
  try {
    ...
    return next(new HttpError('Something went wrong while logging you in, please try again.', 500));
  }
  ...
  try {
    token = jwt.sign({ userId: existingUser._id.toString(), email: existingUser.email }, process.env.SECRET, {
...
const removeFromUserCart = async (req, res, next) => {
  ...
  try {
    ...
    const product = user.cart.items.find((p) => p._id.toString() === productId);
...
const googleContinue = async (req, res, next) => {
  ...
  if (!user) {
    ...
    let refreshTokenExpirationDate;
    try {
      token = jwt.sign({ userId: newUser._id.toString(), email: newUser.email }, process.env.SECRET, {
        expiresIn: process.env.TOKEN_EXPIRE_TIME
      });
      refreshToken = jwt.sign(
        {
  ...
  if (user.password) {
    return next(
      new HttpError(
        'User with such email has already been registered using email and password. ' +
          'Please consider using this method of signing in.',
        403
      )
    );
  }
  ...
  let refreshTokenExpirationDate;
  try {
    token = jwt.sign({ userId: user._id.toString(), email: user.email }, process.env.SECRET, {
      expiresIn: process.env.TOKEN_EXPIRE_TIME
    });
    const existingRefreshToken = await RefreshToken.findOne({ email: userData.email });

    if (!existingRefreshToken || existingRefreshToken?.expirationDate.getTime() <= new Date().getTime()) {
      refreshToken = jwt.sign({ userId: user._id.toString(), email: userData.email }, process.env.REFRESH_TOKEN_SECRET, {
        expiresIn: process.env.REFRESH_TOKEN_EXPIRE_TIME
      });
...
const token = async (req, res, next) => {
  ...
  let jwtToken;
  try {
    jwtToken = jwt.sign({ userId: userData.userId, email: userData.email }, process.env.SECRET, {
      expiresIn: process.env.TOKEN_EXPIRE_TIME
    });
  } catch (e) {
    console.log(e.message);
  ...
  res.status(200);
  res.json({ message: 'Successfully refreshed access token!', accessToken: jwtToken });
};
...
const removeFromCartOffline = async (req, res, next) => {
  ...
  try {
    const productIndex = cart.items.findIndex((p) => p._id.toString() === productId);
...
const verifyUser = async (req, res, next) => {
  ...
  try {
    jwtToken = jwt.sign({ userId: user.id.toString(), email: user.email }, process.env.SECRET, {
      expiresIn: process.env.TOKEN_EXPIRE_TIME
    });
    refreshToken = jwt.sign({ userId: user._id.toString(), email: user.email }, process.env.REFRESH_TOKEN_SECRET, {
      expiresIn: process.env.REFRESH_TOKEN_EXPIRE_TIME
    });
    const newRefreshToken = new RefreshToken({
      refreshToken,
