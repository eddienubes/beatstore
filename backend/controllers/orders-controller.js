...
const createOrderWithPaypal = async (req, res, next) => {
  ...
  let beats;
  try {
    const licensePromises = cartItems.map(async (p) => License.findById(p.licenseId._id));
    const beatsPromises = cartItems.map(async (p) => Beat.findById(p.beatId._id));
    licenses = await Promise.all(licensePromises);
    beats = await Promise.all(beatsPromises);
  } catch (e) {
    return next(new HttpError('Searching for a license or beat went wrong..'), 500);
  }
...
const captureOrderWithPaypal = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError('Searching for a user went wrong..'), 500);
  }
  ...
  try {
    ...
    console.log(e.message);
    return next(new HttpError('Creation order process has failed..'), 500);
  }
...
const createOrderWithWayforpay = async (req, res, next) => {
  ...
  let productName;
  try {
    productName = licenses.map((l, i) => `${beats[i].title} ${l.label}`);
  } catch (e) {
  ...
  const baseSignature = `${purchaseObj.merchantAccount};${purchaseObj.merchantDomainName};${purchaseObj.orderReference};${
    purchaseObj.orderDate
  };${purchaseObj.amount};${purchaseObj.currency};${purchaseObj.productName.join(';')};${purchaseObj.productCount.join(
    ';'
...
const captureOrderWithWayforpay = async (req, res, next) => {
  ...
  const { amount, transactionStatus, orderReference, authCode, cardPan, merchantSignature, reasonCode, clientName, currency } =
    body;
...
const getAllOrders = async (req, res, next) => {
  ...
  try {
    ...
    orders = await Order.find({}, {}, { skip }).sort({ date: -1 }).limit(limit);
  } catch (e) {
    return next(new HttpError('Getting all orders went wrong..'), 500);
  }
...
const getOrdersByUserId = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError('Getting orders by email went wrong..'), 500);
  }
