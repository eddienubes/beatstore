...
router.post(
  '/contact',
  [check('email').isEmail(), check('name').not().isEmpty(), check('subject').not().isEmpty(), check('message').not().isEmpty()],
  usersControllers.contact
);

router.post(
  '/signup',
  [check('username').not().isEmpty(), check('email').isEmail(), check('password').isLength({ min: 6 })],
