...
router.post(
  '/paypal-create',
  [check('email').isEmail().not().isEmpty(), check('cartItems').isArray()],
  ordersController.createOrderWithPaypal
);
router.post(
  '/paypal-capture',
  [check('orderId').isString(), check('token').not().isEmpty()],
...
router.post(
  ...
  [
    ...
    check('transactionStatus').isLength({ min: 4 }),
    check('orderReference').isLength({ min: 6 }),
    check('merchantSignature').isLength({ min: 6 }),
